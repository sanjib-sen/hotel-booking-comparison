        ...
        try {
            ...
            const hotelPromises = bookmarksResponse.map(async (bookmark) => {
                try {
                    ...
                    console.error(`Error fetching hotel with ID ${bookmark.scrapped_item_id}:`, err);
                    return null;
                }
            });

            const hotels = await Promise.all(hotelPromises);
            setBookmarkedHotels(hotels.filter(hotel => hotel !== null) as ScrappedItem[]);
        } catch (err) {
            console.error("Error fetching bookmarked hotels:", err);
            setError(err instanceof Error ? err.message : "Failed to fetch bookmarked hotels");
        } finally {
            setLoading(false);
        }
    }, []);
